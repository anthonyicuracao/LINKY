...
function letsGo() {
    ...
    for (var d = 0; d < allLinks.length; d++) {
        if (!allLinks[d].classList.contains("done")) {
            allLinks[d].style.height = (parseInt(allLinks[d].style.height.replace("px", "")) / h).toString() + "%";
            allLinks[d].style.top = (parseInt(allLinks[d].style.top.replace("px", "")) / h).toString() + "%";
            allLinks[d].style.width = (parseInt(allLinks[d].style.width.replace("px", "")) / w).toString() + "%";
            allLinks[d].style.left = (parseInt(allLinks[d].style.left.replace("px", "")) / w).toString() + "%";
...
function finishFn() {
    ...
    today = mm + "-" + dd + "-" + yyyy;
    var x = "index-" + today.toString() + "-" + new Date().getTime().toString() + ".html";

    download(x, '<!DOCTYPE html><html id="download" lang="en">' + document.getElementById("download").innerHTML + "</html>");
}
...
function pageNumberFn() {
    ...
    document.getElementById("addFormPgNum").setAttribute("value", pageNumberInput.value.toString());
}
